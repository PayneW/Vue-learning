chapter03-以一个小例子为线索# Vue 实例的设计

- 这里是对 `Vue` 实例的整理, 有利于我们直观地观察 `Vue`.
  ```js
    /* - Vue.prototype._init  ****** src/core/instance/index.js */
    // + 每个 Vue 实例都拥有一个唯一的 uid
    vm._uid = uid++; 
    // + 这个表示用于避免 Vue 实例对象被观测(observed)
    vm._isVue = true;
    // + 当前 Vue 实例的初始化选项, 注意: 这是经过 mergeOptions() 后的.
    vm.$options; 
    // + 渲染函数作用域代理
    vm._renderProxy = vm;
    // + 实例本身
    ...
    /* - initEevents(vm) ****** src/core/instance/events.js */
